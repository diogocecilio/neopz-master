...
//recupera os elementos de refencia para criar os patches
...
void TPZAdaptMesh::BuildReferencePatch() {
    ...
    fReferenceCompMesh->CopyMaterials(*tmpcmesh);

    int i,j;
    for (i=0;i<fGeoRef.NElements();i++){

        tmpcmesh->CreateCompEl(fGeoRef[i]);
    }
    ...
    for (int ipatch=0; ipatch<clnel; ipatch++){
        ...
        for (int j=0; j<patchelindex.NElements(); j++){
...
//    int nmat = mesh->MaterialVec().size();
  //  int m;
    ...
    for(el=0; el<nelem; el++) {
        ...
        csint = dynamic_cast<TPZInterpolatedElement *> (cmesh->CreateCompEl(gel));
        if(!csint) continue;
...
//    TPZManVector<REAL,3> xcoarse(3);
    ...
    TPZMaterialDataT<REAL> datacoarse;
...
//        cordsol[0].Zero();
        ...
        const bool hasPhi = true; // geralmente true quando você quer derivadas/phi
        coarse->ComputeSolution(coarse_int_point, datacoarse, hasPhi);
