...
#endif
                    TPZCompEl *clcel = this->CreateCompEl(gel);
                ...
                if (orig_cel){
                    ...
                    if(gclm->IsPatchSon(cloned_gel)) {
                        ...
                        for (j=0;j<orgintel->Reference()->NSides();j++){
                            ...
                            int porder = orgintel->PreferredSideOrder(j);
                            orgintel->Connect(j);
...
*/
    ...
    for (i=0;i<nc;i++)
    {
        ...
        auto &blk_from = fCloneReference->Block();
        auto &blk_to   = this->Block();

        auto &sol_from_sm = fCloneReference->Solution();
        auto &sol_to_sm   = this->Solution();

        // conversão explícita para TPZFMatrix
        TPZFMatrix<STATE> &sol_from = sol_from_sm;
        TPZFMatrix<STATE> &sol_to   = sol_to_sm;

        const int64_t nfrom = blk_from.Size(orgseqnum);
        const int64_t nto   = blk_to.Size(clseqnum);
        const int64_t n = std::min<int64_t>(nfrom, nto);

        const int64_t pfrom = blk_from.Position(orgseqnum);
        const int64_t pto   = blk_to.Position(clseqnum);

        for (int64_t j = 0; j < n; j++) {
            sol_to(pto + j, 0) = sol_from(pfrom + j, 0);
        }
...
void TPZCompCloneMesh::CreateCloneBC() {
    ...
    // auto *bnd = mat->CreateBC (mat,-1000,50,val1,val2);
    //auto *bnd = new TPZBndCond(mat, -1000, 50, val1, val2);

    auto *matt = dynamic_cast<TPZMaterialT<STATE> *>(mat);
    if(!matt){
        DebugStop(); // ou trate erro: material não é TPZMaterialT<STATE>
    }


    TPZBndCond *bnd = matt->CreateBC(matt, -1000, 50, val1, val2);
    if(!bnd) DebugStop();

    // FIX: garanta que o BC aponta para o material volumétrico correto
    bnd->SetMaterial(matt);

    this->InsertMaterialObject(bnd);

    // (opcional) sanity check
    auto *ins = dynamic_cast<TPZBndCond*>(FindMaterial(-1000));
    if(!ins || !ins->Material() || ins->Material()->Id() != matt->Id()) DebugStop();


    // auto *bnd = matt->CreateBC(matt, -1000, 50, val1, val2);
    // this->InsertMaterialObject(bnd);
    //
    // auto *ins = dynamic_cast<TPZBndCond*>(FindMaterial(-1000));
    // std::cout << "BC -1000 mat ptr id = " << (ins && ins->Material() ? ins->Material()->Id() : -999999) << "\n";
...
TPZCompMesh * TPZCompCloneMesh::UniformlyRefineMesh() {
    ...
    for(el=0; el<nelem; el++) {
        ...
        if(bcel) {
            if (bcel->Material()->Id() == -1000) {
                ...
                int sideorder = bcel->PreferredSideOrder(nsides-1);
    ...
    for (i=0;i<nbc;i++){
        ...
        for (is=0;is<nsubel;is++){
            ...
            long indexsub;
            cmesh->CreateCompEl(bcsubgel[is]);
        }
    }
...
void TPZCompCloneMesh::MeshError(TPZCompMesh *fine,TPZVec<REAL> &ervec,
    ...
    if(computesolution) {
        TPZSkylineStructMatrix<REAL> clfstr(fine);
        TPZLinearAnalysis clfan(fine);
...
REAL TPZCompCloneMesh::ElementError(TPZInterpolatedElement *fine, TPZInterpolatedElement *coarse, TPZTransform<REAL> &tr,
                                      ...
                                      TPZStack<TPZGeoEl *> &subels, TPZStack<int> &porders)
{
    ...
    int intorder = cint->PreferredSideOrder(ncon-1);
    ...
    for(side=0; side<nsides; side++) {
        ...
        int p1 = c1->PreferredSideOrder(s1);
        int p2 = c2->PreferredSideOrder(s2);
        //    p1 = p1 > maxp ? maxp : p1;
        //   p2 = p2 > maxp ? maxp : p2;
        ...
        for(c=0; c<5; c++) {
            ...
            auto &blk   = cmesh->Block();
            auto &solSM = cmesh->Solution();
            TPZFMatrix<STATE> &sol = solSM;   // usa o operador de conversão do TPZSolutionMatrix

            const int64_t pos = blk.Position(seq);
            for (d = 0; d < nd; d++) {
                U(counter++,0) = sol(pos + d, 0);
            }
