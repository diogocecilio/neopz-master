...
//  	int numel = numrel*numcel;
    ...
    //PZElasticity2D::TPZElasticity2D(int id, STATE E, STATE nu,
    //                                STATE fx, STATE fy, int planestress)
  	TPZMaterial *meumat = new TPZElasticity2D(1,1.e5,0.2,0,0);
    ...
    for (it=patch.begin(); it!=patch.end(); it++)
    {
        ...
        long index;
        TPZGeoEl *gel = *it;
        TPZCompEl* cel = clonecmesh->CreateCompEl(gel);
        //auto index = cel ? cel->Index() : -1;
