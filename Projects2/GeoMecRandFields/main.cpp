...
REAL HybridBracketedFS(TPZCompMesh* cmesh, REAL coes, REAL atrito,
                       REAL lo, REAL hi, REAL tol_fs_rel, int max_it);
...
REAL AutoRefine(TPZCompMesh* cmesh,REAL coes, REAL atrito, REAL refineAboveVal,int  max_refines,REAL fs_start, REAL fs_step, REAL fs_max)
{
        ...
        for (int ciclo = 0; ciclo < max_refines; ++ciclo) {
                ...
                while (fs <= fs_max + (REAL)1e-12) {
                        ...
                        std::cout << "  [Ramp] FS=" << fs << (ok ? " (ok)" : " (falhou)")
                        << " | iters=" << iters << "\n";
                        if (!ok) break;
                        last_ok = fs;
                        last_ok_iters = iters;
                ...
                std::cout << "[PreRefine] último FS convergente: " << last_ok
                << " (iters=" << last_ok_iters << ")\n";
                       ...
                       REAL lo, REAL hi, REAL tol_fs_rel, int max_it)
{
        ...
        // Garante bracket: lo (OK), hi (FAIL)
        int it=0, dummy=0;
        while (!RunAndAccept(cmesh,coes,atrito,lo,dummy) && it++<8) lo *= (REAL)0.5;
        it=0;
        while (RunAndAccept(cmesh,coes,atrito,hi,dummy) && it++<12) hi *= (REAL)1.5;
        ...
        for (int k=0; k<max_it && rel_gap(lo,hi) > tol_fs_rel; ++k) {
                ...
                int itmid=0;
                bool ok = RunAndAccept(cmesh, coes, atrito, fs, itmid);
                if (ok) {
                        lo = fs;
                        flo = +1.;
