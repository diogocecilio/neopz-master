...
TPZCompMesh * CreateCMesh2 ( TPZGeoMesh *gmesh, int pOrder)
{
    TPZManVector<REAL, 3> bodyforce={0,0,0};
    plasticmat * material=CreateMaterial(0.1e8,0.48,490,20*M_PI/180.,bodyforce,1,1);
...
void SolveSlope()
{
    ...
    TPZManVector<REAL> factors={0,1,3,4,4.01};
    std::string namevtk="block.vtk";
    ApplyLoad( cmesh,factors,loaddir,0,namevtk);
}
...
bool RunAndAccept(TPZCompMesh* cmesh,REAL factor)
{
    auto* bodymat = dynamic_cast<TMatElastoPlaticMC*>(cmesh->FindMaterial(1));
   // auto* bodymat = dynamic_cast<plasticmat*>(cmesh->FindMaterial(1));
                            ...
                            int verbose)
{
    ...
    auto tryFS = [&](REAL fs, int& it,REAL &resu,REAL &resf){ return RunAndAccept(cmesh, fs); };
                               ...
                               std::string vtkfile)
{
    ...
    cmesh->Solution().Zero();
    if (!bodymat) {
        std::cout << "[ArcLength]  material não encontrado  \n";
        DebugStop();
    }
        ...
        while (step < nsteps) {
            ...
            dw.Zero();
            {
                ...
                while (it < maxit_inner && normR > etol_inner)
                {
                    ...
                    std::cout << " iter = "<< it <<"normR" << normR<< " lambda = "<< lambda <<" dl =" << dl<<" Norm(FEXT) = "<< Norm(FEXT)  <<"\n";
                    ++it;
                              ...
                              int mat1D = -1)  // matid para elementos 1D (bordas)
{
    ...
    for (const auto &e : lines) {
        ...
        auto in_left_open = [&](REAL x){ return (x >= 0.0 - tol) && (x < 0.5 - tol); };
...
void ApplyLoadFoot(TPZCompMesh* cmesh,
...
{
    int dim = cmesh->Dimension();
    std::cout << "dimensao da malha computacional = " << dim << std::endl;

    const int nloads = factors.size();
    ...
    // Arquivos de saída
    std::ofstream csv("loadsweep.csv");
    csv << "step,factor,uy,iters,ok\n";
    csv << std::scientific << std::setprecision(15);

    std::ofstream out_uy("uy_load.txt");     // colunas: load_id uy load_factor
    std::ofstream out_res("residuals.txt");  // colunas: load_id load_factor iter res_norm
    out_uy << std::fixed << std::setprecision(10);
    out_res << std::scientific << std::setprecision(12);
    ...
    for (int i = 0; i < nloads; i++)
    {
        // Se quiser fator absoluto use: bcmat->Val2()[loaddir] = factors[i];
        // Se quiser relativo ao load0:   bcmat->Val2()[loaddir] = load0 * factors[i];
        bcmat->Val2()[loaddir] = factors[i];

        TPZStack<STATE> outresF; // norma do resíduo por iteração
        TPZStack<STATE> outresU; // (opcional) norma do incremento

        bool ok = anal.NewtonRaphson(outresF, outresU);

        // número de iterações reportadas
        const int iters = outresF.size();
        ...
        if (iters == 0) {
            // garante uma linha mesmo se o solver não preencher (edge case)
            out_res << (i+1)      << " "
            << factors[i] << " "
            << 0          << " "
            << 0.0        << "\n";
        ...
        // Solução convergida deste incremento
        TPZFMatrix<REAL> solk = anal.Solution();

        // leitura de uy no nó-alvo (ajuste índices)
        REAL uy_inc = UyAtNode(cmesh, 0., 5.);
        uy_acum += uy_inc;

        // Aceita a solução antes de mexer no próximo passo
        anal.AcceptSolution();

        // (opcional) zera a carga para evitar "sujeira" e será reatribuída no próximo loop
        bcmat->Val2()[loaddir] = 0.0;

        std::cout << "Pressure = " << factors[i]
        << "   uy = "   << uy_acum
        << "   iters = " << iters
        << "   ok = "    << ok << "\n";
        ...
        // Pós-processamento VTK
        PostElastoplastic(cmesh, vtkfile, matid, i, dim);
    }

    // fecha (opcional, destrutores já fechariam)
    out_uy.close();
    out_res.close();
    csv.close();
}
// Resolve aa*dl^2 + bb*dl + cc = 0 com escolha por (4.118) e refina por bisseção.
static STATE compute_dlambda_riks_bisect(const TPZFMatrix<STATE>& dwb,
                                         const TPZFMatrix<STATE>& dws,
                                         const TPZFMatrix<STATE>& dw,
                                         STATE L, int& rootIdx)
{
    auto dot = [](const TPZFMatrix<STATE>& a, const TPZFMatrix<STATE>& b)->STATE {
        // presume col-vetores (n x 1)
        const int n = a.Rows();
        STATE s = 0.;
    ...
    const STATE aa = dot(dwb,dwb);
    TPZFMatrix<STATE> t = dw; t += dws;              // t = Δu^(k-1) + δu*
    const STATE bb = 2.0*dot(dwb, t);
    const STATE cc = dot(t,t) - L*L;

    auto f = [&](STATE dl)->STATE {                  // f(dl)=||t + dl*dwb||^2 - L^2
        // Observação: isso é exatamente aa*dl^2 + bb*dl + cc
        return ((aa*dl + bb)*dl + cc);
    };

    const STATE eps = 1e-14;

    // Caso linear (aa ~ 0): cai para -cc/bb
    if (std::abs(aa) < eps) {
        rootIdx = 1;
        return (std::abs(bb) > eps) ? (-cc/bb) : 0.0;
    }

    // --------- raízes estáveis (evita cancelamento) ----------
    STATE disc = bb*bb - 4.0*aa*cc;
    if (disc < 0) disc = 0;                          // clamp numérico
    const STATE sq = std::sqrt(disc);

    // Forma estável:
    // q = -0.5*(bb + sign(bb)*sqrt(disc));   r1 = q/aa;   r2 = cc/q
    const STATE sgn = (bb >= 0.0) ? 1.0 : -1.0;
    const STATE q   = -0.5*(bb + sgn*sq);
    ...
    if (std::abs(q) > eps) {
        ...
        // fallback: use as fórmulas diretas se q≈0
        dl1 = (-bb - sq)/(2.0*aa);
        dl2 = (-bb + sq)/(2.0*aa);
    }
    if (dl2 < dl1) std::swap(dl1, dl2);

    // --------- escolha da raiz pelo seu mesmo "score" (Eq. 4.118) ----------
    auto score = [&](STATE dl)->STATE {
        TPZFMatrix<STATE> x = dw;                    // Δu^(k-1)
        TPZFMatrix<STATE> tmp = dwb;  tmp *= dl;     // dl*dwb
        x += dws;  x += tmp;                         // Δu^(k-1)+δu*+dl dū
        return dot(x, dw);
    };
    const STATE s1 = score(dl1), s2 = score(dl2);

    STATE dl_sel = dl1;
    if (s2 > s1) { dl_sel = dl2; rootIdx = 2; } else { rootIdx = 1; }

    // --------- REFINO por bisseção em torno de dl_sel ----------
    // 1) cria intervalo [a,b] com mudança de sinal de f
    STATE a = dl_sel, b = dl_sel;
    STATE fa = f(a),   fb = fa;

    // passo inicial proporcional à escala:
    STATE delta = std::max<STATE>(1.0, std::abs(dl_sel)) * 1e-6;

    const int max_expand = 40;
    int kexp = 0;
    while (kexp < max_expand && fa*fb >= 0.0) {
        a = dl_sel - delta; fa = f(a);
        b = dl_sel + delta; fb = f(b);
        delta *= 2.0; // expande
        kexp++;
    }

    // Se mesmo assim não conseguiu bracket (raiz dupla ou f ~ 0): retorna dl_sel
    if (fa*fb > 0.0) {
        return dl_sel;
    }
    ...
    // 2) bisseção clássica
    const int   maxit = 60;
    const STATE ftol  = 1e-12;
    for (int it=0; it<maxit; ++it) {
        const STATE m  = 0.5*(a + b);
        const STATE fm = f(m);
        if (std::abs(fm) < ftol) {
            return m;
        }
        // mantém o bracket com sinal oposto
                                ...
                                std::string vtkfile)
{
    ...
    auto* bcmat = dynamic_cast<TPZBndCondT<STATE>*>(cmesh->FindMaterial(-4));

    const REAL load0=-490.;
    bcmat->Val2()[1]=load0;
    an.Assemble();
    TPZFMatrix<STATE> FEXT = an.Rhs();
    ...
    while (step < nsteps) {
        ...
        if (okconv) {
            ...
            const REAL uy = UyAtNode(cmesh, 0., 5.); // <-- NOVO (use seu getter)
            out_ld << step   << " " << lambda << " " << uy << "\n";
            an.AcceptSolution();
            u_acc   = an.Solution();

            // grava λ × uy (ajuste UyAtNode se seu nó/DOF for diferente)
...
// void ApplyLoadFoot(TPZCompMesh* cmesh,TPZManVector<REAL> factors,int loaddir,int indexbc,std::string vtkfile)
// {
//     int dim=cmesh->Dimension();
//
//     cout << "dimensao da malha computacional = " << dim << endl;
...
//     REAL sol=0.;
//     int matid=1;
//     // Abra os arquivos uma vez antes do loop de cargas:
//     std::ofstream out_uy("uy_load.txt");            // colunas: load_id uy load_factor
//     std::ofstream out_res("residuals.txt");         // colunas: load_id load_factor iter res_norm
//     out_uy << std::fixed << std::setprecision(10);
//     out_res << std::fixed << std::setprecision(12);
//     for(int i =0;i< nloads;i++)
//     {
//
//         //bcmat->Val2()[loaddir]=load0*factors[i];
...
//         for (std::size_t k = 0; k < outresF.size(); ++k) {
//             std::cout << "load " << (i+1)
//             << " it "  << (k+1)
//             << " res = " << outresF[k] << "\n";
//         }
//
//         // salva as normas de resíduo por iteração para este incremento
//         for (std::size_t k = 0; k < outresF.size(); ++k) {
//             out_res << (i+1)           << " "
//             << factors[i]      << " "
//             << (k+1)           << " "
...
//         // solução convergida deste incremento
//
//         // exemplo de leitura de uy em um nó (ajuste sua função / índices)
//         REAL uy_inc = UyAtNode(cmesh, 0., 5.);
//
//         // se "sol" é o acumulado, atualize:
//         sol += uy_inc;
//         anal.AcceptSolution();
//         std::cout << "Pressure = " << factors[i]
//         << "   uy = "   << sol << "\n";
