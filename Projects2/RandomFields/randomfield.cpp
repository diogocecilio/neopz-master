...
int main()
{
   ...
   TPZManVector<std::string> scalarnames ( 7 ), vecnames;
    ...
    if (Startfrom ==1)
    {
        ...
        REAL cov=0.3;
        int samples =1000;
        string fieldname="coesaoteste.dat";
        randonanalysis->GenerateNonGaussinRandomField (mean,cov,fieldname,samples );

        mean =30*M_PI/180.;
        cov=0.2;
        fieldname="atritoteste.dat";
        randonanalysis->GenerateNonGaussinRandomField (mean,cov,fieldname,samples );

        randonanalysis->DefineGraphMesh ( 2,scalarnames,vecnames,"filenamefield.vtk" );
        randonanalysis->PostProcess ( 0 );
    }
	//TPZKLStrMatrix *matrix;
	//TPZKLInterpolationSpace *inter;
	//<REAL> cmat;
	//matrix->AssembleC(cmat);
	return 0;
}
...
TPZCompMesh* CreateCompMeshKL (TPZGeoMesh * gmesh )
{
	...
	TPZKarhunenLoeveMat * mat = new TPZKarhunenLoeveMat(id,Lx,Ly,Lz,dim,type,expansionorder);
