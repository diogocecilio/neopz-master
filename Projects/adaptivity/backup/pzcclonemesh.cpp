        ...
        if (reference_gel) {
            ...
            if (cel){
                if(gclm->IsPatchSon(gel)) {        // || gclm->IsNeighBCForPatchSon(gel)) {
                   ...
                   long volside = gel->NSides()-1;
                   long porder = cintel->PreferredSideOrder(volside);
                ...
                if (orig_cel){
                    ...
                    if(gclm->IsPatchSon(cloned_gel)) {
                        ...
                        for (j=0;j<orgintel->Reference()->NSides();j++){
                            ...
                            orgintel->GetgOrder
                           long porder = orgintel->SideOrder(j);
                            orgintel->Connect(j);
                            ...
                            if (logger->isDebugEnabled())
                            {
                               long cloneorder = clintel->SideOrder(j);
...
*/
   ...
   long nc = fCloneReference->NConnects();
    for (i=0;i<nc;i++)
    {
       ...
       long ndoforg 	= fCloneReference->ConnectVec()[i].NDof(*fCloneReference);
       long ndofclone 	= ConnectVec()[fMapConnects[i]].NDof(*this);
...
long nstate = MaterialVec()[0]->NStateVariables();
...
long tmporder = TPZCompEl::gOrder;
 TPZCompEl::gOrder = 10;
...
long orgelcon = fCloneReference->ConnectVec()[orgconid].NElConnected();
long clelcon = ConnectVec()[clconid].NElConnected();
 TPZInterpolatedElement *origel = GetOriginalElement(el);
long constraints =  fCloneReference->ConnectVec()[orgconid].HasDependency();
long orgconstraints = ConnectVec()[clconid].HasDependency();
...
long ncreatedcon = celbc->NConnects();
...
void TPZCompCloneMesh::CreateCloneBC() {
   ...
   long nstate = mat->NStateVariables();
   long dim = mat->Dimension();
   ...
   long ncon = ConnectVec().NElements();
    ...
    /*  long aux = fOriginalConnects.NElements(); */
    ...
    /*  Print(cout); */
//   long tmporder = cmesh->GetDefaultOrder();
//    cmesh->SetDefaultOrder(10);
   long tmporder = GetDefaultOrder();
   ...
   long ibc;
    for(ibc = 0; ibc<nbc; ibc++) {
       ...
       long side = bcelsides[ibc].Side();
       long nsideconnects = intel->NSideConnects(bcelsides[0].Side());
...
TPZCompMesh * TPZCompCloneMesh::UniformlyRefineMesh() {
   ...
   long el, nelem;
    ...
    for(el=0; el<nelem; el++) {
        ...
        if(bcel) {
            if (bcel->Material()->Id() == -1000) {
               ...
               long sideorder = bcel->SideOrder(nsides-1);
    ...
    for(el=0; el<nelem; el++) {
       ...
       long porder = cint->PreferredSideOrder(ncon-1);
        ...
        //   long idbg, indbg, neldbg = subelindex.NElements();
   ...
   long tempgorder = cmesh->GetDefaultOrder();
   ...
   long i;
    for (i=0;i<nbc;i++){
       ...
       long nsubel = bcsubgel.NElements();
       long is;
        for (is=0;is<nsubel;is++){
           long elord = bcporderstack[i]+1;
           long maxp = TPZOneDRef::gMaxP;
...
void TPZCompCloneMesh::MeshError(TPZCompMesh *fine,TPZVec<REAL> &ervec,
   ...
   long el;
    for(el=0; el<numel; el++) {
        ...
        //  long orgcelind = GetOriginalElementIndex(el);
...
REAL TPZCompCloneMesh::ElementError(TPZInterpolatedElement *fine, TPZInterpolatedElement *coarse, TPZTransform &tr,
   ...
   long locmatsize = fine->NShapeF();
   long cormatsize = coarse->NShapeF();
    REAL error = 0.;
    truerror = 0.;
   ...
   long dimension = fine->Dimension();
   long numdof = fine->Material()->NStateVariables();
   ...
   long maxorder = interpolation[0];
    ...
    REAL jacdetcoarse;
   long numintpoints = intrule->NPoints();
    REAL weight;
    // long lin,ljn,cjn;
   long i,j,k;
    ...
    for(int int_ind = 0; int_ind < numintpoints; ++int_ind) {
       ...
       long in;
        for(in=0; in<locnod; in++) {
           ...
           long dfvar = locblock.Size(dfseq);
        ...
        for(in=0; in<cornod; in++) {
           ...
           long dfseq = df->SequenceNumber();
           long dfvar = corblock.Size(dfseq);
    ...
    for (i=0;i<NElements();i++){
       ...
       long orgelindex = GetOriginalElementIndex(anelindex);
        ...
        if (curerror >= minerror){
            ...
            TPZInterpolatedElement *cint = dynamic_cast<TPZInterpolatedElement *> (fCloneReference->ElementVec()[orgelindex]);
           long ncon = cint->NConnects();
           long po = cint->PreferredSideOrder(ncon-1);
                                      ...
                                      TPZStack<TPZGeoEl *> &subels, TPZStack<int> &porders)
{
   ...
   long intorder = cint->SideOrder(ncon-1);
   ...
   long side;
    ...
    for(side=0; side<nsides; side++) {
       ...
       long sidedimension = gel->SideDimension(side);
        if(sidedimension != 1) continue;
        //obtém a ordem do elemento
        //   long level = gel->Level();
       ...
       long nsubel = auxsubelsides.NElements();
       long i;
        for (i=0;i<nsubel;i++){
        ...
        if(subelsides.NElements() != 2 ||
           ...
           long is;
           long nsub = cint->Reference()->NSubElements();
            for (is=0;is<nsub;is++){
   ...
   long gelindex =  gclmesh->Index(gel);
    TPZGeoEl *orgel = gclmesh->ReferenceElement(gelindex);
    
    if (HasDependency) {
       long p = cint->GetPreferredOrder()+1;
        subels.Push(orgel);
        porders.Push(p);
        return;
    }
