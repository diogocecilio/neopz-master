...
int TPZElasticityMaterialSest2D::VariableIndex(const std::string &name)
{
    ...
    if(!strcmp("ETotStressRR",		name.c_str()))  return TPZElasticityMaterialSest2D::ETotStressRR;
    if(!strcmp("ETotStressTT",		name.c_str()))  return TPZElasticityMaterialSest2D::ETotStressRT;
    if(!strcmp("ETotStressRT",		name.c_str()))  return TPZElasticityMaterialSest2D::ETotStressTT;
...
int TPZElasticityMaterialSest2D::NSolutionVariables(int var)
{
    ...
    if(var == TPZElasticityMaterialSest2D::ETotStressRR)     return 1;
    if(var == TPZElasticityMaterialSest2D::ETotStressRT)     return 1;
    if(var == TPZElasticityMaterialSest2D::ETotStressTT)     return 1;
    if(var == 100) return 1;
...
void TPZElasticityMaterialSest2D::Solution(TPZMaterialData &data, int var, TPZVec<STATE> &Solout)
{
    ...
    TPZFNMatrix<9> Rot(3,3,0.), RT(3,3,0.),sigmarad(3,3,0.);
    ...
    TPZFNMatrix<9> SigCartTotal(3,3,0.),SigRadTotal(3,3,0.),temp(3,3,0.);
    SigCartTotal=totalStress;
    RT.Multiply(SigCartTotal,temp);
    ...
    TPZFNMatrix<9> SigCart(3,3,0.),SigRad(3,3,0.);
    SigCart=totalStress;
    RT.Multiply(SigCart,temp);
