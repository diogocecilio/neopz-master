...
#define NOLOG NULL              /* no log file */

#define UFILE_ENVIRON    0
#define UFILE_ABSOLUTE   1
#define UFILE_NOPROGDIR  2
#define UFILE_CANTFIND   4
#define UFILE_PROGDIR    8
#define UFILE_DATADIR    16
...
char *logfilename;              /* Filename for logging */
...
void
logprintf(FILE *logfile, const char *format, ...)
{
  va_list args;

  va_start(args, format);
  vprintf(format, args);
  if (logfile) vfprintf(logfile, format, args);
  va_end(args);
}

void
logputchar(FILE *logfile, char c)
{
  putchar(c);
...
void
logputs(FILE *logfile, const char *s)
{
  fputs(s, stdout);
...
void
showunit(FILE *logfile, struct unittype *theunit)
{
   char **ptr;
   int printedslash;
   int counter = 1;

   logprintf(logfile,num_format.format, theunit->factor);
   ...
   for (ptr = theunit->numerator; *ptr; ptr++) {
      if (ptr > theunit->numerator && **ptr &&
         ...
         if (counter > 1)
           logprintf(logfile,"%s%d", powerstring, counter);
         if (**ptr)
           logprintf(logfile," %s", *ptr);
         counter = 1;
      }
   }
   if (counter > 1)
...
void
showdefinition(FILE *logfile, char *unitstr, struct unittype *theunit)
{
  unitstr = removepadding(unitstr);
  logputs(logfile, deftext);
  ...
  showunit(logfile, theunit);
  logputchar(logfile, '\n');
}


void 
showfunction(FILE *logfile, struct functype *func)
{
  struct unittype unit;
  int not_dimensionless, i;

  if (!func->def) {
    logprintf(logfile," is undefined");
    return;
  }
  ...
  logprintf(logfile,"(%s) = %s", func->param, func->def);
  if (func->domain_min || func->domain_max){
    logputchar(logfile,'\n');
    for(i=strwidth(deftext);i;i--) logputchar(logfile,' ');
    logprintf(logfile, "defined for ");
    if (func->domain_min && func->domain_max) {
      logprintf(logfile, num_format.format, *func->domain_min);
      if (func->dimen && (not_dimensionless || unit.factor != 1)){
        if (isdecimal(*func->dimen))
          logputs(logfile, " *");
        logprintf(logfile," %s",func->dimen);
      }
      logputs(logfile, func->domain_min_open?" < ":" <= ");
    }
    logputs(logfile, func->param);
    if (func->domain_max){
      logputs(logfile, func->domain_max_open?" < ":" <= ");
      logprintf(logfile, num_format.format, *func->domain_max);
    }
    else {
      logputs(logfile, func->domain_min_open?" > ":" >= ");
      logprintf(logfile, num_format.format, *func->domain_min);
    ...
    if (!func->dimen) logprintf(logfile, " (any units)");
  } else if (func->dimen){
    logputchar(logfile,'\n');
    for(i=strwidth(deftext);i;i--) logputchar(logfile,' ');
    if (not_dimensionless) 
      logprintf(logfile,"%s has units %s",func->param, func->dimen);
    else
      logprintf(logfile,"%s is dimensionless",func->param);
  }    
  logputchar(logfile,'\n');
}

void
showtable(FILE *logfile, struct func *fun, int inverse)
{
  int i;

  logprintf(logfile, "%sinterpolated table with points\n",deftext);
  if (inverse){
    int reverse, j;
    ...
    for(i=0;i<fun->tablelen;i++){
      ...
      if (flags.verbose>0)
        logputs(logfile,"\t\t    ");
      logprintf(logfile, "~%s(", fun->name);
      logprintf(logfile, num_format.format, fun->table[j].value);
      if (isdecimal(fun->tableunit[0]))
        logputs(logfile, " *");
      logprintf(logfile," %s",fun->tableunit);
      logputs(logfile, ") = ");
      logprintf(logfile, num_format.format, fun->table[j].location);
      logputchar(logfile,'\n');
    }
  } else {
    for(i=0;i<fun->tablelen;i++){
      if (flags.verbose>0)
        logputs(logfile,"\t\t    ");
      logprintf(logfile, "%s(", fun->name);
      logprintf(logfile, num_format.format, fun->table[i].location);
      logputs(logfile, ") = ");
      logprintf(logfile, num_format.format, fun->table[i].value);
      if (isdecimal(fun->tableunit[0]))
        logputs(logfile, " *");
      logprintf(logfile," %s\n",fun->tableunit);
    }
...
void
showfuncdefinition(FILE *logfile, struct func *fun, int inverse)
{
  if (fun->table)  /* It's a table */
    showtable(logfile, fun, inverse);
  else { 
    logprintf(logfile,"%s%s%s", deftext,inverse?"~":"", fun->name);
    if (inverse)
      showfunction(logfile, &fun->inverse);
    else
      showfunction(logfile, &fun->forward);
  }
}


void
showunitlistdef(FILE *logfile, struct wantalias *alias)
{
  logprintf(logfile, "%sunit list, ",deftext);
...
int
showfunc(FILE *logfile, char *havestr, struct unittype *have, struct func *fun)
{
   int err;
   char *dimen;
   ...
   if (err) {
     if (err==E_BADFUNCARG){
       logprintf(logfile, "conformability error");
       if (fun->table)
         dimen = fun->tableunit;
       ...
       if (!dimen)
         ...
         if (*dimen==0)
           dimen = "1";
         logprintf(logfile,": conversion requires dimensions of '%s'\n",dimen);
         if (flags.verbose==2) logprintf(logfile,"\t%s = ",havestr);
         else if (flags.verbose==1) logputchar(logfile,'\t');
         showunit(logfile,have);
         if (flags.verbose==2) logprintf(logfile, "\n\t%s = ",dimen);
         else if (flags.verbose==1) logprintf(logfile, "\n\t");
         else logputchar(logfile,'\n');    /* coverity[check_return] */
         parseunit(&want, dimen, 0, 0);    /* coverity[check_return] */
         completereduce(&want);         /* dimen was already checked for */
         showunit(logfile,&want);       /* errors so no need to check here */
         logputchar(logfile,'\n');
         }
     } else if (err==E_NOTINDOMAIN)
       logprintf(logfile,"Value '%s' is not in the function's range\n",havestr);
     else
       logprintf(logfile,"Function evaluation error (bad function definition)\n");
     return 1;
   }
   if (flags.verbose==2)
     logprintf(logfile,"\t%s = %s(", havestr, fun->inverse.param);
...
void
showconformabilityerr(FILE *logfile, 
                      char *havestr,struct unittype *have,
                      char *wantstr,struct unittype *want)
{
  logprintf(logfile,"conformability error\n");
  if (flags.verbose==2)
    logprintf(logfile,"\t%s = ",havestr);
  else if (flags.verbose==1)
    logputchar(logfile,'\t');
  showunit(logfile,have);
  if (flags.verbose==2)
...
void
showunitname(FILE *logfile, double value, char *unitstr, int printnum)
{
  ...
  if (printnum && !(rnd_value == 1 && isdecimal(*unitstr)))
    logprintf(logfile, num_format.format, value);

  if (strpbrk(unitstr, "+-"))   /* show sums and differences of units */
    logprintf(logfile," (%s)", unitstr);   /* in parens */
           ...
           char *wantstr,struct unittype *want)
{
   ...
   if (compareunits(have, want, ignore_dimless)) {
        ...
        *dest=0;
        if (flags.strictconvert || compareunits(&invhave, want, ignore_dimless)){
          showconformabilityerr(logfile, havestr, have, wantstr, want);
          return -1;
        }
        if (flags.verbose>0)
          logputchar(logfile,'\t');
        logprintf(logfile,"reciprocal conversion\n");
        have=&invhave;
        doingrec=1;
   } 
   if (flags.verbose==2) {
   ...
   if (flags.verbose==2) 
     logprintf(logfile,"\t%s%s%s = ",left,havestr,right);
   else if (flags.verbose==1)
     logputs(logfile,"\t* ");
   if (flags.verbose==2)
     showunitname(logfile, have->factor / want->factor, wantstr, PRINTNUM);
   else
     logprintf(logfile,num_format.format, have->factor / want->factor);
   ...
   if (!flags.oneline){
     if (flags.verbose==2) 
       ...
       logputchar(logfile,'\n');
     logprintf(logfile,num_format.format, want->factor / have->factor);
     if (flags.verbose==2) {
       logputchar(logfile,')');
       showunitname(logfile, 0,wantstr, NOPRINTNUM); 
     }
   }
   logputchar(logfile,'\n');
   return 0;
...
/*
  If HOME directory cannot be found, returns NULL. Otherwise:
    With checkonly == 0, returns NULL if file doesn't exist.
    With checkonly != 0, returns filename even if it doesn't exist, and
    doesn't give error message
*/
char *
personalunitsfile(int checkonly, int *exists)
{
  FILE *testfile=0;
  ...
  *exists = 0;
  ...
  if (filename && *filename){
    ...
    if (testfile){
      ...
      *exists = 1;
      ...
      fprintf(stderr, "%s: cannot open units file '%s' specified in MYUNITSFILE environment variable. ", progname, filename);
      perror((char *) NULL);
      return 0;
    }
  }
  /*
      ...
      HOMEDRIVE and HOMEPATH, and USERPROFILE
  */
  homedir = getenv("HOME");
#ifdef _WIN32
  if (!homedir) {
  ...
  if (testfile){
    ...
    *exists = 1;
    ...
    if (errno==EACCES) {
      fprintf(stderr,"%s: cannot read units file '%s'.  ",progname,filename);
      perror(0);
    }
    free(filename);
    return 0;
...
usage()
{
   ...
   printf("Usage: %s [options] ['from-unit' 'to-unit']\n",progname);
   fprintf(stdout, "\
\n\
Options:\n\
    ...
    -I, --info           show version, files, and program properties; exit\n",
DEFAULTPRECISION,DEFAULTTYPE,DEFAULTPRECISION);
...
printversion()
{
  ...
  if (flags.verbose == 2) {
    putchar('\n');
    unitsfile = getenv("MYUNITSFILE");
    if (unitsfile)
      printf("Environment variable MYUNITSFILE set to '%s'\n", unitsfile);
    else
      puts("Environment variable MYUNITSFILE not set");
  }

  unitsfile = personalunitsfile(1, &exists);
  if (unitsfile) {
      ...
      if (compareunits(unit, unit+1, ignore_dimless)){
        if (printerror){
          ...
          *(strchr(firstunitstr, UNITSEPCHAR)) = '\0';
          ...
          unitstr = removepadding(unitstr);
          showpointer(promptlen);
          showconformabilityerr(NOLOG, firstunitstr, unit, unitstr, unit+1);
...
int
showunitlist(FILE *logfile,char *havestr, struct unittype *have, char *wantstr)
{
  ...
  while (wantstr) {
    ...
    */
    ...
    if (!flags.verbose){
      if (!firstunit) 
        logputchar(logfile, UNITSEPCHAR);
      logprintf(logfile, num_format.format,value);
      value_shown=1;
    } else { /* verbose case */
      if (value != 0) {
        if (value_shown) /* have already displayed a number so need a '+' */
          logputs(logfile, " + ");
        showunitname(logfile, value, wantstr, PRINTNUM);
      value_shown=1;
      }
    }
    freeunit(&want);
  ...
  if (!value_shown) {  /* provide output if every value rounded to zero */
    ...
    if (isdecimal(*lastunitstr))
      logputs(logfile, "* ");
    logputs(logfile, lastunitstr);
  }

  if (round_dir != 0) {
    if (flags.verbose){
      if (round_dir > 0)
        logprintf(logfile," (rounded up to nearest %s) ", lastunitstr);
      else
        logprintf(logfile," (rounded down to nearest %s) ", lastunitstr);
    } else 
      logprintf(logfile,"%c%c", UNITSEPCHAR, round_dir > 0 ?'-':'+');   
  }
  logputchar(logfile,'\n');
  return 0;
} /* end showunitlist */
...
#  endif
   ...
   if (!unitsfiles[0]){
     ...
     if (!unitsfile)
       ...
       unitsfiles[1] = personalunitsfile(0, &file_exists);
       unitsfiles[2] = 0;
     }
   }
   ...
   if (!interactive) {
      ...
      if (strchr(wantstr, UNITSEPCHAR)){
        if (showunitlist(NOLOG, havestr, &have, wantstr))
          exit(EXIT_FAILURE);
        else
          exit(EXIT_SUCCESS);
      }
      if (showanswer(NOLOG, havestr,&have,wantstr,&want))
         exit(EXIT_FAILURE);
      else
         exit(EXIT_SUCCESS);
   } else {       /* interactive */
      ...
      for (;;) {
         ...
         if ((alias = aliaslookup(removepadding(havestr)))){
           showunitlistdef(logfile, alias);
           continue;
         }
         if ((funcval = isfunction(havestr))){
           showfuncdefinition(logfile, funcval, FUNCTION);
           continue;
         }
         if (*removepadding(havestr) == '~' && 
             (funcval = isfunction(removepadding(havestr)+1))){
         ...
         if (isblankstr(wantstr))
           showdefinition(logfile, havestr,&have);
         else if (strchr(wantstr, UNITSEPCHAR))
           showunitlist(logfile, havestr, &have, wantstr);
         else if ((funcval = isfunction(wantstr)))
           showfunc(logfile, havestr, &have, funcval); /* Clobbers have */
         else {
           showanswer(logfile, havestr,&have,wantstr, &want);
           freeunit(&want);
         }
         freeunit(&have);
      }
...
int
convertGNUUnits(const char *unitsFileStr, const char *fromStr, const char *toStr, double *multiply, double *divide)
{
   static struct unittype have, want;
   char *havestr=0, *wantstr=0;
   int readerr;
...
//   printf("%s", progdir);
   ...
   //unitsfiles[0] = "/home/raul/untitled/units-2.11/definitions.units";
   unitsfiles[0] = unitsFileStr;
...
//   }
   ...
   queryhave = querywant = "";   /* No prompts are being printed */
    ...
    //RETURNING VALUES
    *multiply = have.factor / want.factor;
    *divide = want.factor / have.factor;
