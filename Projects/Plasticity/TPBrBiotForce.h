...
public:
  ...
  TPBrBiotForce(const TPBrBiotForce &cp) : TPZFunction<STATE>(cp), fRwell(cp.fRwell), fRreservoir(cp.fRreservoir), fPwell(cp.fPwell), fPreservoir(cp.fPreservoir), fBiot(cp.fBiot), fConstant(cp.fConstant)
  {
	...
	virtual void Execute(const TPZVec<REAL> &x, TPZVec<STATE> &f, TPZFMatrix<STATE> &df)
    {
        ...
        f[0] = -fBiot*(fPreservoir-fPwell)*x[0]/(r2*log(fRreservoir/fRwell));
        f[1] = -fBiot*(fPreservoir-fPwell)*x[1]/(r2*log(fRreservoir/fRwell));
        
        df.Redim(1, 1);
        //df(0,0) = fBiot*(fPwell+fConstant*log(sqrt(r2)/fRwell/fRwell)/2.);
        df(0,0) = fBiot*(fPwell+((fPreservoir-fPwell)/log(fRreservoir/fRwell))*log(sqrt(r2)/fRwell));
    }
