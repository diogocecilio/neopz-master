...
void readnodes(TPZGeoMesh *gmesh, std::string filename);
void readels(TPZGeoMesh *gmesh, std::string filename);
TPZCompMesh * FootCMesh(TPZGeoMesh * gmesh, int pOrder);
#include <iostream>
#include <fstream>
#include <string>
using namespace std;
...
int main(int argc, char *argv[])
{
	...
	std::string filenamenodes = "strip-nodes.txt";
	readnodes(gmesh, filenamenodes);
	std::string filenameels = "strip-els.txt";
	readels(gmesh, filenameels);
	gmesh->BuildConnectivity();
	//UniformRefinement(gmesh, 1);
	//gmesh->BuildConnectivity();
	//UniformRefinement(gmesh, 1, 1);
	...
	std::ofstream Dummyfile("GeometicMeshsdf.vtk");
	TPZVTKGeoMesh::PrintGMeshVTK(gmesh, Dummyfile, true);
	TPZCompMesh *cmesh = FootCMesh(gmesh, porder);
...
void readnodes(TPZGeoMesh *gmesh, std::string filename)
{
	...
	TPZGeoNode node;
	std::vector<TPZGeoNode> vecnodes;
	...
	REAL cox, coy, coz;
		...
		cout << "vec[0]" << vecnodes[0].Id() << endl;
		int nnodes = vecnodes.size();
		gmesh->NodeVec().Resize(nnodes);
		//cout << "size = " << gmesh->NodeVec().NElements() << endl;
		for (int i = 0; i < nnodes; i++)
		{
			int nodeid = vecnodes[i].Id();
			gmesh->NodeVec()[nodeid] = vecnodes[i];
		}
...
void readels(TPZGeoMesh *gmesh, std::string filename)
{
	...
	int elid, top1, top2, top3, top4, matid;
	...
	TopoLine[0] = 2;
	TopoLine[1] = 3;
	new TPZGeoElRefPattern< pzgeom::TPZGeoLinear>(elid, TopoLine, rigthmat, *gmesh);

	int upmat = 4;
	elid = 6;
	TopoLine[0] = 4;
	TopoLine[1] = 7;
	new TPZGeoElRefPattern< pzgeom::TPZGeoLinear>(elid, TopoLine, upmat, *gmesh);

	int leftmat = 5;
	elid = 7;
	TopoLine[0] = 1;
	TopoLine[1] = 4;
	new TPZGeoElRefPattern< pzgeom::TPZGeoLinear>(elid, TopoLine, leftmat, *gmesh);
...
void UniformRefinement(TPZGeoMesh * gMesh, int nh, int ElId)
{
	for ( int ref = 0; ref < nh; ref++ ){
		TPZVec<TPZGeoEl *> filhos;
		long n = gMesh->NElements();
