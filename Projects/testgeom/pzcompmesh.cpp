...
int main() {
	...
	REAL deltat = 0.01;
	REAL gamma = 1.4;
	TPZMaterial *mat = new TPZEulerConsLaw(1,deltat,gamma,2,SUPG_AD);
...
//	cmesh->InsertMaterialObject((TPZMaterial *)(mat->CreateBC(mat,-3,TPZEulerEquation::EFreeSlip,val1,val2)));
	...
	TPZFMatrix<STATE> bcval1(nstates,nstates,0.),bcval2(nstates,1,0.);
	cmesh->InsertMaterialObject((TPZMaterial *)(mat->CreateBC(mat,-1,0,bcval1,bcval2)));   // Dirichlet condition -  (wall)
	// Inserts Neumann boundary condition - input flux
	cmesh->InsertMaterialObject((TPZMaterial *)(mat->CreateBC(mat,-2,1,bcval1,bcval2)));   // Neumann condition - fluxo livre
	// Inserts Neumann boundary condition - output free flux
	cmesh->InsertMaterialObject((TPZMaterial *)(mat->CreateBC(mat,-3,1,bcval1,bcval2)));   // Neumann condition - fluxo livre
...
//	TPZCreateApproximationSpace::SetAllCreateFunctionsDiscontinuous();
	...
	TPZMaterial *mat3 = new TPZEulerEquation(1,gamma);
	cmesh3->InsertMaterialObject(mat3);
	// Inserts Dirichlet boundary condition - wall then zero values for all variables
	int nstates3 = mat3->NStateVariables();
	TPZFMatrix<STATE> val_1(nstates3,nstates3,0.),val_2(nstates3,1,0.);
